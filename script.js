...
function nasaApiRequest(){
    ...
    $form.addEventListener('submit', async function(e){
        ...
        //picks information from API
        const response = await fetch(`https://api.nasa.gov/planetary/apod?api_key=2tBDqFu2qkhFP0UYmTkyfe69GmgH2Dq8UwccXWdb&date=${$date.value}`)
        ...
        if(json.code){
            ...
            $apod.innerHTML = `
                <img class="imgbig rounded-3 img-fluid" id="imgbig" data-hd="${json.hdurl}" src="${json.url}">
                <div class="ms-4">
                    <h3>${json.title}</h3>
        ...
        $body.addEventListener('click', function(e){
            if (e.target.classList.contains('imgbig')){
                ...
                $overlay.innerHTML = `<div class="imgOverlay"><img class="img-fluid" src=${hd}></div>`
    ...
    for (let i = 0; i < favorites.length; i++){
        html.push(`
            <div class="fav-info border d-flex flex-wrap p-3 mt-3">
                <img class="img-small rounded-3 img-fluid" src="${favorites[i].url}">
                <div class="ms-4">
